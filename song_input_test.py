...
def get_song(song: str, artist: str, filename: str) -> str | None:
    ...
    with open(filename, encoding="utf8") as csvfile:
        ...
        list_of_songs = [row[0].lower() for row in rows]
        list_of_artists = [row[1].lower() for row in rows]

    close_songs = difflib.get_close_matches(song.lower(), list_of_songs)  # Song titles similar to user's input
    ...
    for i in range(len(list_of_artists)):
        if list_of_songs[i] in close_songs and list_of_artists[i] in close_artists:
            ...
            score = difflib.SequenceMatcher(None, list_of_songs[i], song.lower()).ratio()
